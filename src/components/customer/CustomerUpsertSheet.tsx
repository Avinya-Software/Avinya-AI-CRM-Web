...
const CustomerUpsertSheet = ({
  ...
  useEffect(() => {
    ...
    if (customer) {
      ...
      setExistingKycFiles(
        customer.kycFiles ? customer.kycFiles.split(",").filter(Boolean) : []
      );
    } else {
  ...
  const validate = () => {
    ...
    else if (!mobileRegex.test(form.primaryMobile)) e.primaryMobile = "Invalid mobile number";

    if (!form.email.trim()) e.email = "Email is required";
    else if (!emailRegex.test(form.email)) e.email = "Invalid email address";

    if (form.dob && form.anniversary) {
      if (new Date(form.anniversary) <= new Date(form.dob))
        e.anniversary = "Anniversary date must be after date of birth";
    }
      ...
      <div className="fixed top-0 right-0 h-screen w-[420px] bg-white z-[70] shadow-2xl flex flex-col animate-slideInRight">
        ...
        <div className="flex-1 overflow-y-auto px-6 py-4 space-y-4">
          ...
          <Input
            ...
            max={new Date().toISOString().split("T")[0]}
            onChange={(v: string) => setForm({ ...form, anniversary: v })}
            disabled={isReadOnly}
          />
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="flex gap-2">
                        ...
                        {/* Preview — always visible (view action) */}
                        <button
                        ...
                        {/* Download — always visible (view action) */}
                        <button
                        ...
                        {/* Delete — only visible if user has customer:delete */}
                        {canDelete && (
                          <button
                            onClick={async () => {
          ...
          {/* ===== KYC UPLOAD — hidden in read-only mode ===== */}
          {!isReadOnly && (
            <div>
              <label className="text-sm font-medium">Add KYC Documents</label>
              <input
                type="file"
                multiple
                accept=".pdf,.jpg,.png"
                onChange={(e) =>
                  setKycFiles(e.target.files ? Array.from(e.target.files) : [])
                }
        ...
        <div className="px-6 py-4 border-t flex gap-3">
          ...
          {!isReadOnly && (
            <button
              disabled={saving}
              className="flex-1 bg-blue-900 text-white rounded-lg py-2 flex items-center justify-center gap-2 hover:bg-blue-800 transition disabled:opacity-50"
              onClick={handleSave}
            >
              {saving && <Spinner />}
...
const Input = ({ label, required, value, error, type = "text", min, max, onChange, disabled }: any) => (
  <div>
